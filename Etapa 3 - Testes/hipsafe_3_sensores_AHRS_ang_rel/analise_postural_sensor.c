...
// Verifica se a posição é perigosa:
// - Flexão de quadril: |pitch relativo| entre mpu0 e mpu1 > 75°
// - Rotação interna de quadril: |roll relativo| entre mpu0 e mpu1 > 45°
bool posicaoPerigosa() 
{
    float pitch_rel = g_pitch[0] - g_pitch[1];
    float roll_rel = g_roll[0] - g_roll[1];
    if (fabsf(pitch_rel) > 75.0f) {
        // Flexão de quadril perigosa
        return true;
    }
    if (fabsf(roll_rel) > 45.0f) {
        // Rotação interna de quadril perigosa
        return true;
    }
    return false;
}
